...
export function RuleTooltip({ content }) {
  ...
  return (
    <span ref={ref} className="relative inline-flex items-center align-middle">
      ...
      {open && (
        <div id={tipId} role="tooltip" className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-72 sm:w-80 p-3 rounded-xl shadow-xl border text-xs leading-relaxed max-h-64 overflow-y-auto bg-white/95 backdrop-blur-xl border-white/60 text-slate-700 dark:bg-slate-900/95 dark:border-white/10 dark:text-slate-200 origin-bottom motion-safe:animate-[popIn_0.15s_ease-out]">
          {content}
          <div aria-hidden="true" className="absolute top-full left-1/2 -translate-x-1/2 -mt-px w-2 h-2 rotate-45 bg-white/95 border-b border-r border-white/60 dark:bg-slate-900/95 dark:border-white/10" />
        </div>
      )}
    </span>
  );
