...
- **覆盖全档位**：竞赛优秀奖、特等奖、体育 4-8 名、破省级纪录、出版专著、发明专利、5 档荣誉表彰等

### 目标分推荐器（算法实验内容）
- **背包 DP 求解**：给定当前得分与目标分，反向求出**总时长最少**的加分组合
- **难度分级**：每项候选标注 normal / hard / very-hard，可一键过滤高难度
- **一键排除**：推荐结果中不想要的项目一键剔除，自动重新推荐，持久化到 localStorage
- **我的现状问卷**：6 维度声明（任职 / 学科竞赛 / 论文 / 文艺 / 体育 / 荣誉）后只推荐"不超过当前档位"的项目

### 体验与导出
- **数据持久化**：自动保存到 localStorage，刷新不丢；支持**一键重置全部**（带确认弹窗）
- **评分规则提示**：每项旁的 `?` 图标，hover / click 查看详细标准
- **得分可视化**：三模块条形图直观展示提升空间
- **证书式评分图导出**：环形仪表盘 + 三模块色卡片 + **逐项明细**（每个得分项逐条列出）+ 横幅校徽 + 嵌入 Noto Sans SC 字体，可保存 PNG 或打印 / PDF
- **主题自动跟随系统**：默认跟随系统深 / 浅色并**实时切换**，手动切换则记为偏好并持久化；手机 / 桌面端响应式适配
- **细腻交互动效**：生成推荐后平滑滚动定位、分数 count-up 滚动、推荐项级联入场、结果高亮脉冲，配回到顶部悬浮按钮；全程尊重系统「减少动态效果」偏好
...
```
src/
├── App.jsx                        # 主组件（状态管理 + 主题/重置/布局组合，~215 行）
...
│   ├── ExportModal.jsx            # 导出弹窗 + Canvas 证书式评分图绘制
│   ├── ConfirmDialog.jsx          # 通用可访问确认弹窗（一键重置等）
...
└── hooks/
    ...
    ├── useCountUp.js              # 数字 count-up 补间 Hook（reduced-motion 友好）
    └── useLocalStorage.js         # 本地持久化（输入 + 推荐器现状/排除项 + 主题偏好）
```
